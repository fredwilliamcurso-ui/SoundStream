...
export async function purchaseCoins(
  ...
  try {
    // 1. Check for duplicate reference ID to prevent payment fraud and replay attacks
    const txnQuery = query(
      collection(db, "coin_transactions"), 
      where("stripeSessionId", "==", referenceId),
      where("status", "==", "completed")
    );
    const existingTxns = await getDocs(txnQuery);
    if (!existingTxns.empty) {
      throw new Error("This payment reference has already been credited.");
    }

    // 2. Perform transaction-safe coin credit
    const wallet = await getOrCreateWallet(userId);
    const newBalance = (wallet.balance || 0) + pack.coins;
    const newTotalPurchased = (wallet.totalPurchased || 0) + pack.coins;
    ...
    const transactionRecord: CoinTransaction = {
      id: transactionId,
      userId,
      type: "purchase",
      amount: pack.coins,
      stripeSessionId: referenceId,
      packageName: `${pack.coins} SoundStream Coins`,
      status: "completed",
      createdAt: new Date().toISOString()
    };

    // Save transaction and update wallet atomic-like
    await setDoc(txnRef, transactionRecord);
    await updateDoc(walletRef, updatedWallet);

    // Sync user stats or notifications if needed
    try {
      await addDoc(collection(db, "notifications"), {
        userId,
        receiverId: userId,
        title: "⚡ Coins Credited!",
        message: `Your account has been credited with ${pack.coins} SoundStream Coins. Keep supporting creators!`,
    ...
    return { success: true, newBalance, transaction: transactionRecord };
  } catch (error) {
    handleFirestoreError(error, OperationType.WRITE, `coin_transactions/${transactionId}`);
    throw error;
  }
}
