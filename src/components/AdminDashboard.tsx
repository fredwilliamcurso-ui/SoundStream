    ...
    const unsubSubscriptions = onSnapshot(collection(db, "subscriptions"), (snapshot) => {
      ...
      if (data.length === 0) {
        setSubscriptionsList([
          { subscriptionId: "s_sub1", userId: "demo3", plan: "Premium Individual", status: "active", price: 9.99, currency: "USD", billingCycle: "monthly", startedAt: new Date(Date.now() - 259200000).toISOString(), expiresAt: new Date(Date.now() + 2332800000).toISOString(), paymentProvider: "stripe" },
          { subscriptionId: "s_sub2", userId: "demo2", plan: "Creator Pro", status: "active", price: 19.99, currency: "USD", billingCycle: "monthly", startedAt: new Date(Date.now() - 518400000).toISOString(), expiresAt: new Date(Date.now() + 2073600000).toISOString(), paymentProvider: "paypal" }
        ]);
      } else {
                      ...
                      <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                        {securityAlerts.map(alert => (
                          <div key={alert.id} className="p-3 bg-red-950/20 border border-red-500/15 rounded-xl space-y-1">
                            <div className="flex items-center justify-between">
                              <span className="font-mono text-[9px] font-bold text-red-400 uppercase tracking-wide">{alert.level} ALERT</span>
                              <span className="font-mono text-[8px] text-zinc-500">{alert.time}</span>
                            </div>
                            <p className="text-[10px] text-zinc-300 font-sans leading-relaxed">{alert.message}</p>
                          </div>
                        ))}
