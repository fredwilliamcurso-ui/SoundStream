...
export default function SoundStreamCreatorStudio({ creatorId, onNavigateToUpload }: CreatorStudioProps) {
  ...
  const [profileSoundstream, setProfileSoundstream] = useState<string>("");
      ...
      try {
        ...
        if (!docSnap.exists()) {
          ...
          const initialSettings = {
            ...
            socialLinks: { instagram: "artist_insta", soundstream: "artist_soundstream", youtube: "artist_yt" },
  ...
  useEffect(() => {
    ...
    const unsubSettings = onSnapshot(doc(db, "creator_settings", creatorId), (snapshot) => {
      if (snapshot.exists()) {
        ...
        setProfileSoundstream(data.socialLinks?.soundstream || "");
    ...
    try {
      await updateDoc(doc(db, "creator_settings", creatorId), {
        ...
        socialLinks: {
          ...
          soundstream: profileSoundstream,
      ...
      if (!artistSnap.empty) {
        await updateDoc(doc(db, "artists", artistSnap.docs[0].id), {
          artistName: profileArtistName,
          bio: profileBio,
          soundstreamUsername: profileSoundstream
        });
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-9 space-y-6">
            ...
            {activeTab === "content" && (
              ...
              >
                {/* Upload Section */}
                <div className="bg-[#0b0b0f] border border-white/5 p-6 rounded-3xl space-y-4 relative overflow-hidden">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-zinc-400 max-w-xl leading-relaxed">
                      To publish lossless music tracks, HD video mixes, or independent shorts, use the central <strong>Upload Hub</strong>. Your uploads will undergo automated copyright verification, smart metadata parsing, and lossless flac layer decoding automatically.
                    </p>
                  </div>
                  ...
                  {onNavigateToUpload && (
                    <div className="pt-2">
                      <button
                        ...
                        className="px-5 py-2.5 rounded-xl bg-indigo-650 hover:bg-indigo-600 text-white font-bold font-mono text-[10px] uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2"
                      >
                ...
                <form onSubmit={handleSaveSettings} className="space-y-6">
                  ...
                  <div className="bg-zinc-900/20 border border-white/5 p-6 rounded-3xl space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="font-mono text-[9px] uppercase tracking-wider text-zinc-500 font-bold block mb-1">Linked Soundstream Username</label>
                        <input
                          type="text"
                          value={profileSoundstream}
                          onChange={(e) => setProfileSoundstream(e.target.value)}
                          className="w-full bg-zinc-950 border border-white/5 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500 font-mono"
                        />
                      </div>
