...
import { getOrCreateWallet, purchaseCoins } from "../lib/coinsService";
...
export default function SoundStreamWallet({ userId, currentUser }: SoundStreamWalletProps) {
  ...
  useEffect(() => {
    ...
    getOrCreateWallet(userId).catch((err) => {
      handleFirestoreError(err, OperationType.WRITE, `wallets/${userId}`, userId);
    });
    ...
    const walletUnsub = onSnapshot(doc(db, "wallets", userId), (snap) => {
      ...
      handleFirestoreError(error, OperationType.GET, `wallets/${userId}`, userId);
    });
    ...
    const txUnsub = onSnapshot(q, (snap) => {
      ...
      handleFirestoreError(error, OperationType.LIST, "coin_transactions", userId);
      setLoading(false);
    });
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-emerald-500/90 leading-relaxed mt-1">
                  Your secure Stripe payment completed successfully and your virtual coins have been added to your wallet. You should receive a confirmation receipt email at your registered email address shortly. Enjoy streaming!
                </p>
                {checkoutSessionId && (
