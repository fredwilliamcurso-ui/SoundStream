...
import { getOrCreateWallet, sendGift, purchaseCoins } from "../lib/coinsService";
...
export default function GiftStoreModal({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "wallets", senderId), (snap) => {
      ...
      console.warn("Failed to subscribe to wallets:", error);
    });
  ...
  const renderIcon = (iconName: string, active: boolean) => {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(iconName)) {
      return <span className="text-3xl select-none filter drop-shadow-md leading-none">{iconName}</span>;
    }
    
    switch (iconName) {
      case "Music": return <Music className={`${size} ${color}`} />;
      case "Rose": return <span className="text-2xl">🌹</span>;
  ...
  return (
    <div className="fixed inset-0 bg-black/85 z-55 flex items-center justify-center p-4 backdrop-blur-sm">
      <motion.div
        ...
        className="bg-[#0b0b0f] border border-white/10 rounded-3xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] shadow-2xl relative"
      >
        {/* Top Header */}
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400 uppercase tracking-widest leading-none mt-0.5">
          ...
          <button 
            onClick={onClose}
            className="p-1.5 rounded-full bg-white/5 hover:bg-white/10 transition text-zinc-400 hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {showCoinStore ? (
          ...
          <div className="flex-1 overflow-y-auto p-5 bg-[#0c0c11] flex flex-col gap-4 min-h-[300px]">
                ...
                return (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setSuccessMessage(null);
                      try {
                        const mockSessionId = `mock_ref_${Date.now()}`;
                        const res = await purchaseCoins(senderId, pkg, mockSessionId);
                        if (res.success) {
                          setSuccessMessage(`Successfully purchased ${pkg.coins} Coins!`);
                          setTimeout(() => {
                            setSuccessMessage(null);
                            setShowCoinStore(false);
                          }, 1500);
                        ...
                        setErrorMessage(err?.message || "Failed to complete simulated checkout.");
                      } finally {
                        setPurchasingPackId(null);
                      }
                    }}
                    className={`group relative p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-indigo-500/50 hover:bg-white/[0.04] transition duration-300 flex flex-col justify-between text-left min-h-[120px] ${
                      isPurchasing ? "border-indigo-500 animate-pulse" : ""
                    }`}
                  >
                    {pkg.badge && (
                      <span className="absolute top-2.5 right-2.5 px-1.5 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest bg-indigo-600/20 text-indigo-400 border border-indigo-500/10">
                        {pkg.badge}
                      </span>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[8px] text-zinc-500 uppercase font-semibold mt-0.5 tracking-wider">
                          Coins
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between mt-4 pt-2 border-t border-white/5 w-full">
                      <span className="font-mono text-xs font-black text-indigo-400">${pkg.price}</span>
                      <span className="text-[9px] font-black uppercase tracking-widest text-white bg-indigo-600 group-hover:bg-indigo-500 px-2 py-1 rounded-lg transition-all">
          ...
          <>
            {/* Categories Scroller */}
            <div className="flex items-center gap-1.5 px-5 py-2.5 bg-[#121216] overflow-x-auto border-b border-white/5 no-scrollbar shrink-0">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition shrink-0 border ${
                    activeCategory === cat 
            ...
            {/* Content Panel / Grid */}
            <div className="p-5 overflow-y-auto flex-1 min-h-[250px] bg-[#0c0c11] grid grid-cols-3 gap-3">
              {filteredGifts.map((gift) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative p-3 rounded-2xl flex flex-col items-center justify-center text-center transition-all duration-300 border ${
                      isSelected 
                  ...
                  >
                    {/* Rarity Tag */}
                    <span className={`absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded-md text-[7px] uppercase font-bold tracking-widest border ${getRarityBadgeColor(gift.rarity)}`}>
                      {gift.rarity}
                    </span>

                    {/* Gift Visual Icon */}
                    <div className={`p-3 rounded-full mb-2.5 transition-transform duration-300 ${
                      isSelected ? "bg-indigo-600/30 scale-110" : "bg-white/5 group-hover:scale-105"
                    }`}>
                      {renderIcon(gift.icon, isSelected)}
                    </div>

                    <span className="font-extrabold text-xs text-white leading-none tracking-tight">
                      {gift.name}
                    </span>
        ...
        <div className="p-5 bg-[#0e0e14] border-t border-white/5 flex flex-col gap-3 shrink-0">
          ...
          <AnimatePresence mode="wait">
            {errorMessage && (
              <motion.div 
                ...
                exit={{ opacity: 0 }}
                className="flex flex-col gap-2 p-3.5 bg-rose-950/20 rounded-2xl border border-rose-900/30 text-center w-full"
              >
                ...
                {errorMessage.includes("Insufficient") && (
                  <button
                    ...
                    className="mx-auto bg-amber-500 hover:bg-amber-400 text-black text-[10px] font-black px-4 py-1.5 rounded-xl uppercase tracking-widest transition"
                  >
                    Buy More Coins
                  </button>
                )}
            ...
            {successMessage && (
              <motion.p 
                ...
                animate={{ opacity: 1, y: 0 }} 
                exit={{ opacity: 0 }}
                className="text-xs text-emerald-400 bg-emerald-950/20 px-3 py-1.5 rounded-xl border border-emerald-900/30 text-center font-semibold leading-tight animate-pulse"
              >
                ✨ {successMessage}
              </motion.p>
            )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-[#1b1b24] p-2 rounded-2xl border border-white/5">
              ...
              <div>
                ...
                <p className="font-mono text-base font-black text-white flex items-center gap-1">
            ...
            <button
              ...
              className="flex items-center gap-1 text-[10px] font-extrabold text-indigo-400 hover:text-indigo-300 uppercase tracking-wider font-mono bg-indigo-500/5 px-3 py-1.5 rounded-xl border border-indigo-500/20 hover:bg-indigo-500/10 transition"
            >
              {showCoinStore ? "View Gifts" : "Buy Coins"} <ArrowRight className="w-3 h-3" />
            </button>
          </div>
          ...
          {!showCoinStore && (
            <button
              ...
              className={`w-full py-3.5 rounded-2xl font-extrabold uppercase tracking-widest text-xs transition duration-300 flex items-center justify-center gap-2 border ${
                !selectedGift 
            ...
            >
              {loading ? (
                ...
                <span>Select a Virtual Gift</span>
              )}
            </button>
          )}
        </div>
