...
// Helper to map alphanumeric Firebase UIDs deterministically to positive 32-bit integers for Agora
const stringToNumericUid = (uidString: string): number => {
  let hash = 0;
  ...
  return positiveHash === 0 ? 999999 : positiveHash;
};
...
export default function LiveStreamingDashboard({
  ...
  useEffect(() => {
    ...
    queueAgoraAction("Join Room", async () => {
      ...
      try {
        ...
        const fetchAgoraToken = async (channelName: string, uid: number | null, role: "publisher" | "subscriber"): Promise<string | null> => {
          try {
            const uidParam = uid !== null ? `&uid=${uid}` : "";
            const res = await fetch(`/api/agora/token?channelName=${encodeURIComponent(channelName)}${uidParam}&role=${role}`);
            if (!res.ok) {
            ...
            console.warn("[Agora Token] Could not fetch token from server, falling back to null (AppID-only mode):", e);
            return null;
          }
        };
        ...
        // Reconnection & Network Change Handling
        agoraClient.on("connection-state-change", (curState: string, revState: string, reason: string) => {
          console.log(`[Agora] Connection state changed from ${revState} to ${curState} due to ${reason}`);
          if (curState === "RECONNECTING") {
            console.warn("[Agora] Network connection lost, reconnecting to channel...");
          } else if (curState === "CONNECTED") {
            console.log("[Agora] Network connection restored/established successfully.");
          }
        });

        // Token renewal handlers
        agoraClient.on("token-privilege-will-expire", async () => {
          console.log("[Agora] Token is about to expire, renewing token...");
          const numericUid = currentUser ? stringToNumericUid(currentUser.uid) : null;
          const isHost = activeStream?.creatorId === currentUser?.uid;
          const freshToken = await fetchAgoraToken(activeStream.id, numericUid, isHost ? "publisher" : "subscriber");
          if (freshToken) {
            try {
              await agoraClient.renewToken(freshToken);
              console.log("[Agora] Token renewed successfully before expiration.");
            } catch (err) {
              console.error("[Agora] Failed to renew token:", err);
            }
          }
        });

        agoraClient.on("token-privilege-did-expire", async () => {
          console.warn("[Agora] Token expired. Re-fetching and renewing...");
          const numericUid = currentUser ? stringToNumericUid(currentUser.uid) : null;
          const isHost = activeStream?.creatorId === currentUser?.uid;
          const freshToken = await fetchAgoraToken(activeStream.id, numericUid, isHost ? "publisher" : "subscriber");
          if (freshToken) {
            try {
              await agoraClient.renewToken(freshToken);
              console.log("[Agora] Token renewed successfully after expiration.");
            } catch (err) {
              console.error("[Agora] Failed to renew expired token:", err);
            }
          }
        });

        // Set proper initial role: the channel host joins as "host", others as "audience"
        const isHost = activeStream?.creatorId === currentUser?.uid;
        await agoraClient.setClientRole(isHost ? "host" : "audience");
        console.log(`[Agora] Setting initial client role to: ${isHost ? "host" : "audience"}`);
        ...
        // Map deterministic numeric UID
        const numericUid = currentUser ? stringToNumericUid(currentUser.uid) : null;

        // Fetch secure token
        console.log("[Agora] Fetching secure token from server...");
        const token = await fetchAgoraToken(activeStream.id, numericUid, isHost ? "publisher" : "subscriber");

        // Join Agora room using Stream ID as channel name
        await agoraClient.join(appId, activeStream.id, token, numericUid);
        console.log("[Agora] Successfully joined channel:", activeStream.id, "as UID:", numericUid, "with token:", token ? "SECURE_TOKEN" : "NULL_FALLBACK");
    ...
    queueAgoraAction("Update Publish State", async () => {
      ...
      try {
        if (shouldPublish) {
          ...
          // 3. Local video preview is managed directly by <video ref={videoRef} /> to avoid clashing DOM operations
          // with the Agora player. No need to call localVideoTrackRef.current.play() here.
  ...
  // Handle Android background/foreground lifecycle behavior and network recovery
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (document.visibilityState === "visible") {
        console.log("[Lifecycle] App returned to foreground. Performing health check on stream connection...");
        
        const client = agoraClientRef.current;
        if (activeStream && agoraJoined && client) {
          console.log("[Lifecycle] Agora current connection state:", client.connectionState);
          
          // 1. Recover connection if disconnected
          if (client.connectionState === "DISCONNECTED") {
            console.warn("[Lifecycle] Agora client is disconnected. Attempting to rejoin/recover connection...");
          }
          
          // 2. Recover host tracks if ended by system suspension
          const isHost = activeStream.creatorId === currentUser?.uid;
          if (isHost && localStream) {
            const tracks = localStream.getTracks();
            const anyTrackEnded = tracks.some(t => t.readyState === "ended");
            if (anyTrackEnded) {
              console.warn("[Lifecycle] Host media tracks were ended by the OS while backgrounded. Re-acquiring camera/microphone...");
              stopLocalMedia();
  ...
  }, [agoraJoined, activeStream, localStream, currentUser]);
  ...
  // Reconnect host to their active live stream
  useEffect(() => {
    if (!currentUser || activeStream) return;
    const existing = streams.find(
      s => s.creatorId === currentUser.uid && s.status === "live"
    );
    if (existing) {
